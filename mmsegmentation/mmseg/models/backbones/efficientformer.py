'''
File: /sensei-fs/users/zwei/Dev/DevFramework/IMPSegmentation/fast_seg/models/efficientformerv1_c.py
Project: /sensei-fs/users/zwei/Dev/DevFramework/IMPSegmentation/fast_seg/models
Created Date: Monday December 26th 2022 12:59:07 am
Author: zwei
-----
Last Modified:
Modified By:
...
Dev Notes: Added efficientformer v1, customized for 320x320 
'''
"""
...
class Attention(torch.nn.Module):
    def __init__(self, dim=384, key_dim=32, num_heads=8,
                 attn_ratio=4,
                 resolution=10):
        #TODO: this resolution will change: 224->7, 320->10
        super().__init__()
        ...
        # print('pdb @ Attention'); import pdb; pdb.set_trace()
...
class Meta3D(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4.,
                 ...
                 drop=0., drop_path=0.,
                 use_layer_scale=True, layer_scale_init_value=1e-5, resolution=16):
...
def meta_blocks(dim, index, layers,
                ...
                use_layer_scale=True, layer_scale_init_value=1e-5, vit_num=1, resolution=16):
    blocks = []
...
class EfficientFormer(BaseModule):
    ...
    def __init__(self, layers, embed_dims=None,
        ...
        for i in range(len(layers)):
            stage = meta_blocks(embed_dims[i], i, layers,
                                ...
                                use_layer_scale=use_layer_scale,
                                layer_scale_init_value=layer_scale_init_value,
                                vit_num=vit_num, resolution=self.resolution)
        ...
        if self.fork_feat:
            ...
            # following v2 design, not needed norm any more
            # for i_emb, i_layer in enumerate(self.out_indices):
            #     if i_emb == 0 and os.environ.get('FORK_LAST3', None):
            #         layer = nn.Identity()
            #     else:
            #         # normlayer is changed...
    ...
    def init_weights(self):
        if self.init_cfg is None:
            # do nothing, just normal initializqation 
            ...
            print("{} calling super init_weights".format(self.__class__.__name__))
            super().init_weights()
    ...
    def forward_tokens(self, x):
        # after the norm being removed
        # this part is subject to optimization, all the resizes should not be used.
        ...
        for idx, block in enumerate(self.network):
            ...
            if self.fork_feat and idx in self.out_indices:
                ...
                if len(x.size()) != 4:
                    x = x.transpose(1, 2).reshape(B, C, self.resolution, self.resolution)
                # x_out = norm_layer(x)
                outs.append(x)
        if self.fork_feat:
    ...
    # no specific freeze stage for efficientformer
    # new for mmsegmentation1.0
    # def train(self, mode=True):
    #     super().train(mode)
    #     self._freeze_stages()
